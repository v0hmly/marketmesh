...
Проверка `GOWORK=off` использует уже опубликованные версии обычных зависимостей.
После MM-100 protobuf-контракты являются результатом генерации текущего checkout:
для них изолированная проверка MM-101 использует временный `-modfile` с локальным
`replace` только `api/gen/go`. Перед проверкой API генерируется закреплёнными
инструментами; временные файлы удаляются и не изменяют зарегистрированные
`go.mod`/`go.sum`. Все модули по-прежнему проходят test/build с `GOWORK=off` и
`-mod=readonly`; для остальных зависимостей проверяются закреплённые версии.
...
`api/proto` является источником публичных, внутренних и туннельных protobuf-схем. `backend/api/gen/go` и `frontend/gen` содержат игнорируемые Git результаты генерации и ручные manifests/README. Результаты не коммитятся и не редактируются вручную (MM-100); `task generate` восстанавливает их из исходников. `backend/api/tunnel` содержит общий fail-closed декодер и структурные ограничения туннельных кадров; он зависит только от `backend/api/gen/go` и protobuf runtime и не реализует состояние gateway или маршрутизацию.
...
Docker-образ сервиса собирается с контекстом корня монорепозитория, чтобы сборка могла сгенерировать контракты из `api/proto` в build stage и получить платформенные модули. Dockerfile при этом находится рядом с сервисом или в согласованном каталоге поставки и копирует только необходимые файлы.
