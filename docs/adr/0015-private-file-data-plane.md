...
Предложение принять реализацию в `dev` при выключенном API и сохранить
непройденные эксплуатационные условия в MM-88/MM-89 описано в
[ограниченной приёмке MM-43](../security/files-dev-acceptance.md).
Оно требует отдельного явного подтверждения владельца; до него статус этого
ADR и исходные условия приёмки не меняются.
