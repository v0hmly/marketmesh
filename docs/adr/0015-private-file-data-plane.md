...
- Статус: Принято для текущего dev по решению владельца от 22.09.2026
- Дата: 2026-09-21
- Задача: MM-43
- Уточняет: ADR-0009
...
## Временная модель владельца и аудит доступа

В MM-43 `tenant_id == subject_id`: каждый подтверждённый Auth subject имеет
личное пространство файлов. Это временное ограничение модели, а не поддержка
организационных tenants. Auth пока не выдаёт независимый tenant и подтверждённое
членство; нельзя брать их из тела запроса, заголовков или неподтверждённых claims.
Files проверяет подпись assertion и актуальную сессию через Auth на каждой
операции, после чего сам формирует пару owner `(subject, subject)`.
Общий доступ, делегирование и владение от имени продавца/организации отсутствуют.

Переход к независимым tenants требует отдельного ADR, доверенного контракта Auth
для membership/ролей и явной миграции существующих owner-пар, квот и ключей
идемпотентности. Простая замена `tenant_id` значением из нового запроса недопустима:
до миграции существующие записи продолжают принадлежать личному пространству.
Обязательны отрицательные тесты доступа между tenants и повторов старых uploads.

Состав control-аудита ограничен операцией, кодом результата, случайным `file_id`,
workload/pod и проверенными `tenant_id`, `subject_id`, `session_id`. Последний —
идентификатор сессии для корреляции с Auth, не credential. Принципал записывается
только после успешной проверки подписи и текущего состояния сессии, в том числе
при дальнейшем отказе по scope или владельцу. Для неаутентифицированных запросов
он отсутствует: извлекать subject/session из непроверенного JWT нельзя.
Это служебные псевдонимные идентификаторы для аудита безопасности с ограниченным
доступом; email, имя пользователя/файла, содержимое, URL, JWT, cookie и сырые
ошибки зависимостей в аудит не входят. SQL `files.audit` отдельно фиксирует
системные переходы состояний и не заменяет аудит пользовательского доступа.
...
## Приёмка текущего dev

Владелец `v0hmly` принял интеграцию Files в текущий dev без требования
обязательного выключения API и с явным остаточным риском CVE; пересмотр
до 22.10.2026. Полная область — [решение MM-43](../security/files-dev-acceptance.md).
Исходные security-инварианты этого ADR сохраняются. CVE исправляются в MM-88
и связанной MM-67, а не объявляются устранёнными этим решением.

[Локальный DC E2E](../testing/files-dc-e2e.md) должен подтвердить stop обеих VM
каждого DC, fencing/promotion, метаданные и READY-байты, Auth/owner/session,
fail-closed запись без remote peer и resume после восстановления. На одном
физическом хосте это проверка логических отказов с сохранёнными дисками;
будущие физические fault domains требуют отдельной проверки MM-89.
Merge и завершение MM-43 по-прежнему требуют успешных проверок и актуального
независимого ревью. Само наличие манифестов не считается пройденным E2E.
