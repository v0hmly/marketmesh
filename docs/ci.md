...
MM-100: API-код генерируется до Go/frontend-проверок, а `api:generate-check`
сравнивает две независимые генерации без эталонных файлов в Git. CSS токенов
создаётся командой `task design:generate`. Layout-проверка запрещает generated-файлы
в индексе. Используется существующий один job, без новой матрицы и сервисов CI.

MM-87: существующий frontend-шаг проверяет оба приложения и общие пакеты,
отрицательные тесты импортов и бюджеты production-сборки со всеми возможностями.
Реальные SSO/PG, браузерные сценарии и сетевой периметр проверяются локально
через `task staff:browser`, `task staff:perimeter`, `task frontend:bundles:browser`.
Дополнительных hosted jobs, контейнерных сервисов и платных интеграций не добавлено.
