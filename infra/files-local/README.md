...
Минимальная жизнь sandbox — 11 с до перезапуска: теоретически менее 5,5 документа
в минуту на один контейнер, фактически ниже с учётом обработки и I/O. Подробные
[лимиты пропускной способности](../../backend/services/files/README.md#лимиты-и-эксплуатация)
учитывают также последовательный worker и его паузу между job.
...
`control_e2e.py` использует прежний ограниченный password/session-контракт Auth:
в одноразовой БД применяются только миграции 000001–000003, а email выключен.
Это не проверка новых почтовых сценариев: они проходят полный E2E с Mailpit в
`infra/account-local`. Отключение email на схеме 000004 остаётся запрещённым.
Порт временного account-проекта — `19443`; его можно изменить через
`FILES_CONTROL_PORT`, чтобы тест не занимал порт работающего локального кабинета.

Основной стенд не запускает публичные маршруты. Только отдельный `control_e2e.py`
включает их на loopback-порту в одноразовом проекте. `control.json` содержит
только приватную конфигурацию S3/DB для тестов, без включённого listener.
Параметры control/Auth/gateway перечислены в [README Files](../../backend/services/files/README.md).
