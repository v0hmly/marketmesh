...
Это интеграционный тест Files; полного сценария браузер→Auth→туннель→Files
и потери целого DC он не заменяет.
...
Публичные маршруты в этом стенде не запускаются. `control.json` содержит
