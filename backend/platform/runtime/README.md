...
Минимальная интеграция находится в [`backend/services/user/internal/app`](../../services/user/internal/app). `main` создаёт signal-aware root context и делегирует запуск `app.Run`; `internal/app` явно создаёт config, logger, telemetry, health, transport adapters и `Runner`. Бизнес-логики в примере нет.
