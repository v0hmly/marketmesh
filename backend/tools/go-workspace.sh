...
readonly BACKEND_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
readonly REPOSITORY_ROOT="$(cd "${BACKEND_ROOT}/.." && pwd)"
...
module_packages() {
  ...
  (
    cd "${BACKEND_ROOT}/${module_directory}"
...
verify_module_paths() {
  ...
  for module in "${MODULES[@]}"; do
    ...
    actual_path="$(awk '$1 == "module" { print $2 }' "${BACKEND_ROOT}/${module_directory}/go.mod")"
...
verify_go_versions() {
  ...
  actual_version="$(awk '$1 == "go" { print $2 }' "${BACKEND_ROOT}/go.work")"
  if [[ "${actual_version}" != "${GO_VERSION}" ]]; then
    echo "Invalid Go version in backend/go.work: expected ${GO_VERSION}, got ${actual_version}" >&2
    return 1
  fi

  for module in "${MODULES[@]}"; do
    module_directory="${module%%|*}"
    actual_version="$(awk '$1 == "go" { print $2 }' "${BACKEND_ROOT}/${module_directory}/go.mod")"
...
verify_workspace_modules() {
  ...
  for module in "${MODULES[@]}"; do
    ...
    if ! grep -Fqx "${expected_path}" <<<"${actual_modules}"; then
      echo "Module ${expected_path} is absent from backend/go.work" >&2
      return 1
    fi
  done

  if [[ "$(wc -l <<<"${actual_modules}" | tr -d ' ')" -ne "${#MODULES[@]}" ]]; then
    echo "backend/go.work contains a module not declared in backend/tools/go-workspace.sh" >&2
    return 1
  fi
}
...
verify_no_local_replaces() {
  ...
  for module in "${MODULES[@]}"; do
    module_directory="${module%%|*}"
    if grep -Eq '=>[[:space:]]+(\.{1,2}/|/)' "${BACKEND_ROOT}/${module_directory}/go.mod"; then
