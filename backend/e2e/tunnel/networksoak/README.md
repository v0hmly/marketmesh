...
`backend/platform/testkit`, а ownership fault lifecycle остаётся в исходных пакетах:
...
До публикации task-ветки standalone-модуль `backend/e2e/tunnel` не может получить
локальную версию нового `backend/platform/testkit/networkchaos` без запрещённого
`replace`. Поэтому bridge сейчас проверяется из корневого `backend/go.work`; после push
