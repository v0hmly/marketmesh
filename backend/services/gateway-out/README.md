...
`Client.Component` подключает tunnel к `backend/platform/runtime`. Gateway Out не создаёт входящий listener в DMZ.
...
`backend/platform/grpc.ClientConfig.DisableRetries`; прозрачный повтор транспорта
