...
readonly TEST_PROJECT="marketmesh-auth-registration-test-$$"
...
trap cleanup EXIT
if [[ -z "$PROVIDED_IMAGE" ]]; then compose build auth-registration-integration; fi
compose run --rm --no-deps fixture-generator
compose up --detach --wait --wait-timeout 120 postgres postgres-ro nats
compose run --rm --no-deps auth-registration-integration
