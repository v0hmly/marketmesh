...
MM-101 включает email-код по умолчанию у новых аккаунтов. Подтверждение письма в
браузере регистрации сразу выдаёт сессию; для этого отдельная HttpOnly cookie
должна совпасть с HMAC-привязкой challenge. В другом браузере ссылка только
подтверждает почту. Требуется добавочная миграция `000005_registration_login`;
порядок обновления и отката описан в [Auth email](../../../docs/security/auth-email.md).
