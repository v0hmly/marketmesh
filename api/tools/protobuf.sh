...
readonly PROTOC_GEN_ES_SOURCE="${REPO_ROOT}/frontend/node_modules/.bin/protoc-gen-es"
...
install_es_plugin() {
  ...
  configured_version="$(
    cd "${REPO_ROOT}"
    node -p "require('./frontend/package.json').devDependencies['@bufbuild/protoc-gen-es']" 2>/dev/null || true
  )"
  if [[ "${configured_version}" != "${PROTOC_GEN_ES_VERSION}" ]]; then
    printf 'frontend/package.json должен фиксировать @bufbuild/protoc-gen-es версии %s\n' \
...

  {
    ...
    printf '%s\n' 'exec "${script_dir}/../frontend/node_modules/.bin/protoc-gen-es" "$@"'
  } >"${PROTOC_GEN_ES_BIN}"
...
run_easyp() {
  (
    ...
    EASYPPATH="${CACHE_DIR}/easyp" \
      ...
      "${EASYP_BIN}" --cfg "${REPO_ROOT}/api/easyp.yaml" "$@"
  )
}
...
verify_pins() {
  ...
  generator_version="$(
    cd "${REPO_ROOT}"
    node -p "require('./frontend/package.json').devDependencies['@bufbuild/protoc-gen-es']"
  )"
  runtime_version="$(
    cd "${REPO_ROOT}"
    node -p "require('./frontend/gen/package.json').dependencies['@bufbuild/protobuf']"
  )"

  if [[ "${generator_version}" != "${PROTOC_GEN_ES_VERSION}" ]]; then
    printf 'Версия protoc-gen-es не совпадает с api/tools/protobuf-versions.env.\n' >&2
    return 1
  fi
  if [[ "${runtime_version}" != "${PROTOBUF_ES_VERSION}" ]]; then
    printf 'Версия @bufbuild/protobuf не совпадает с api/tools/protobuf-versions.env.\n' >&2
    return 1
  fi
  if ! grep -Fq "github.com/googleapis/googleapis@${GOOGLEAPIS_REVISION}" "${REPO_ROOT}/api/easyp.yaml"; then
    printf 'Ревизия googleapis в api/easyp.yaml не совпадает с api/tools/protobuf-versions.env.\n' >&2
    return 1
  fi
  if [[ ! -s "${REPO_ROOT}/api/easyp.lock" ]] || \
    ! grep -Fq "${GOOGLEAPIS_REVISION}" "${REPO_ROOT}/api/easyp.lock"; then
    printf 'api/easyp.lock отсутствует или не фиксирует ожидаемую ревизию googleapis.\n' >&2
    return 1
  fi
}
...
generate_to() {
  local go_out="$1"
  local ts_out="$2"
  [[ "${go_out}" = /* ]] || go_out="${REPO_ROOT}/${go_out}"
  [[ "${ts_out}" = /* ]] || ts_out="${REPO_ROOT}/${ts_out}"
  local relative_go_out relative_ts_out
  # EasyP 0.16.1 joins output paths to its operation root, even absolute paths.
  relative_go_out="$(node -p 'require("node:path").relative(process.argv[1], process.argv[2])' "${REPO_ROOT}/api" "${go_out}")"
  relative_ts_out="$(node -p 'require("node:path").relative(process.argv[1], process.argv[2])' "${REPO_ROOT}/api" "${ts_out}")"

  clean_generated "${go_out}" "${ts_out}"
  (
    cd "${REPO_ROOT}/api"
    EASYPPATH="${CACHE_DIR}/easyp" \
      PROTO_TOOLS_BIN="${BIN_DIR}" \
      PROTOC_GEN_ES_PATH="${PROTOC_GEN_ES_BIN}" \
      PROTO_GO_OUT="${relative_go_out}" \
      PROTO_TS_OUT="${relative_ts_out}" \
      "${EASYP_BIN}" --cfg "${REPO_ROOT}/api/easyp.yaml" generate
  )
  generate_error_details "${ts_out}"
  normalize_generated_text "${ts_out}"
}
...
generate_error_details() {
  ...
  module_version="$(awk '$1 == "github.com/googleapis/googleapis" { print $2 }' "${REPO_ROOT}/api/easyp.lock")"
...
breaking_snapshot() (
  ...
  cp "${source_repo}/${config_path}/easyp.lock" "${snapshot_dir}/" || return
  ...
  awk '$1 == "against_git_ref:" {$0 = "  against_git_ref: baseline"} {print}' \
    "${source_repo}/${config_path}/easyp.yaml" >"${snapshot_dir}/easyp.yaml" || return
  (
...
PROTO
  cp "${REPO_ROOT}/api/easyp.lock" "${test_dir}/easyp.lock"
