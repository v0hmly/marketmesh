...
Сначала изменяется схема, затем выполняются `task api:lint`, `task api:breaking` и `task api:generate`. Файлы в `backend/api/gen/go` и `frontend/gen` являются результатом генерации и вручную не исправляются.
...
Wire-правила v1 строгие: неизвестные frame types, поля и значения enum отклоняются общим пакетом `backend/api/tunnel/v1`. Новое поле, которое protobuf breaking check считает совместимым, нельзя использовать без согласованной capability или новой версии протокола.
