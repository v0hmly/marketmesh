...
func TestUnconfiguredRouteIsRejectedWithoutInternalCall(t *testing.T) {
	...
	for _, route := range []contractv1.RouteId{contractv1.RouteId_ROUTE_ID_AUTH_LOGIN, contractv1.RouteId_ROUTE_ID_USER_BROWSER_GET_ME, contractv1.RouteId_ROUTE_ID_USER_BROWSER_UPDATE_ME} {
		t.Run(route.String(), func(t *testing.T) {
			class := routeTrafficClass(route)
			pki := newTestPKI(t)
