...
const (
	...
	maxKeys           = 64
	readScope         = "user:profile:read"
	writeScope        = "user:profile:write"
)
...
func New(client authv1.AuthInternalServiceClient, config Config) (*Verifier, error) {
	if client == nil || strings.TrimSpace(config.Issuer) == "" || config.MaxTTL < time.Second || config.MaxTTL > 5*time.Minute || config.MaxTTL%time.Second != 0 || config.Timeout <= 0 {
		return nil, errors.New("user authsession: invalid configuration")
	}
	if config.Clock == nil {
...
func (v *Verifier) Verify(ctx context.Context, token string) (identity.Principal, error) {
	...
	verifier, err := sessionassert.NewVerifier(v.config.Issuer, "user", keys, sessionassert.WithLeeway(0), sessionassert.WithVerifierMaxTTL(v.config.MaxTTL), sessionassert.WithVerifierClock(v.config.Clock))
	if err != nil {
		return denied()
	}
	...
	subject, err := profile.NewSubjectID(raw)
	if err != nil {
		return denied()
	}
	...
	return identity.Principal{SubjectID: subject, CanRead: claims.HasScope(readScope), CanWrite: claims.HasScope(writeScope), CanReadAddresses: claims.HasScope("user:addresses:read"), CanWriteAddresses: claims.HasScope("user:addresses:write")}, nil
}
...
func parseKeys(raw string, clock func() time.Time) (keySet, error) {
	invalid := errors.New("user authsession: invalid signing keys")
